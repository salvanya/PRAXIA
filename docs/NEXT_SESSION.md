...
⚠️ **ACCIÓN PREVIA — ANTES de arrancar el próximo slice (pedido explícito del usuario, 2026-07-03): ARREGLAR el env de `mypy`.** `mypy app/` CRASHEA con `TypeError: Integer exceeds 64-bit range` (`mypy/util.py:json_dumps`, al escribir cache), afecta los 48 archivos, reproducible desde cache limpia — **NO es defecto del código** (cada archivo pasa aislado con `mypy <file> --follow-imports=skip`). Pasos: (1) **`pip install -U mypy`** (bug conocido de serialización de int en la cache; suele resolverse actualizando); (2) re-correr `cd backend && python -m mypy app/` → debe dar limpio; (3) si persiste, bisectá qué módulo lo dispara (`mypy` sobre subsets) o localizá el int literal ≥2^64. El gate `mypy app/` del DoD está ROTO hasta resolverlo. **Recién con el gate verde, seguí con el Context Manager completo.**

2. **FASE 1 (MVP conversacional) CERRADA** — 10 slices en `origin`. **FASE 2 EN CURSO: Slice 1 (eval offline como GATE, merge `e4bb30f`, pusheado) y Slice 2 (MEMORIA DE LARGO PLAZO + reflexión + Context Manager MÍNIMO, merge LOCAL `5b328b5`, 2026-07-03) CERRADOS.** NO reabras nada de eso; el detalle vive en la memoria del proyecto. **Slice 2 mergeado (`5b328b5`, `--no-ff`) y PUSHEADO a `origin`.**
...
Estado (2026-07-03, main PUSHEADO a `origin`):
...
Alcance restante de Fase 2 (orden acordado): **Context Manager COMPLETO** (`running_summary` + presupuesto de tokens + prefijo-estable para KV-cache; el mínimo ya está — `app/context.py` es el punto de ensamblado) → **memoria RICA: update/delete/contradicción** (DESPUÉS del Context Manager, decisión usuario 2026-07-03 — hoy NO se puede pisar/borrar una memoria) → **DSPy** (MIPROv2/GEPA contra el golden set) · **caching** (semántico + embeddings) · **trazas Phoenix** · **guardrails ENDURECIDOS** (inyección con llm-guard, output safety, audit log — blueprint línea 556; el seam `G_OUT` va antes de `reflect`).

Punto de arranque a confirmar conmigo (preguntame ANTES de escribir código). Sugerencia de orden:
- **CONTEXT MANAGER COMPLETO (el próximo, recomendado):** `running_summary` (resumen incremental de turnos viejos) + presupuesto de tokens + orden prefijo-estable para el KV-cache. Continúa directo lo de Slice 12 (el mínimo ya inyecta memorias; `context.py` es el hogar).
- **MEMORIA RICA: update/delete/contradicción (INMEDIATAMENTE DESPUÉS del Context Manager — decisión del usuario 2026-07-03):** hoy NO se puede pisar/borrar una memoria (dedup solo casi-idéntico ≥0.9; las contradictorias conviven y el recall inyecta ambas). Enfoques: recall pesado por recencia/`salience` + chequeo de contradicción en `reflect` (UPDATE/supersede) + comando "olvidá/corregí que…" + UI de gestión/tighten del gate (guardó "los profesionales activos son…" = dato efímero). Tabla ya prepped (`id`/`salience`/`last_used_at`/`source`).
- Después: GUARDRAILS ENDURECIDOS (inyección/output-safety/audit; `G_OUT` antes de `reflect`) · DSPy · caching · Phoenix — optimización/seguridad/observabilidad (rinden con el eval-gate para medir).
