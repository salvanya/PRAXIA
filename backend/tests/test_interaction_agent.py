...
async def test_happy_path_builds_action(monkeypatch) -> None:
    ...
    assert p["summary"] == "Ana confirmó el turno"  # se guarda crudo
    assert p["content"].startswith("Llamé a Ana")  # se guarda crudo
    assert p["occurred_at"] == "2026-06-28T14:30:00+00:00"
    assert p["source"] == "agente"
    assert "Ana López" in pa["summary"]
    assert p["content"] in pa["summary"]  # WYSIWYG: la tarjeta muestra el content crudo
    assert pa["summary"] != p["summary"]  # la tarjeta (content) != la columna summary
