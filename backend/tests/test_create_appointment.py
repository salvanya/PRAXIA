...
@pytest.mark.integration
async def test_find_practitioners_by_name_returns_active() -> None:
    from seed_demo import seed_demo

    await seed_demo()
    pid = get_settings().practice_id
    rows = await db.find_practitioners_by_name(pid, "", limit=3)
    assert rows and all("id" in r and "full_name" in r for r in rows)
