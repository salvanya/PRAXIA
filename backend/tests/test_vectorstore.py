...
def _chunk(text: str, idx: int, doc: str) -> Chunk:
    return Chunk(
        text=text, page=1, chunk_index=idx, document_id=doc, title="Protocolo", doc_type="protocolo"
    )
...
async def test_search_filters_by_practice():
    ...
    assert all(h["document_id"] == "doc-a" for h in hits), "only practice A's data may appear"
